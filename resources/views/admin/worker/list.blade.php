    ...
    <div class="d-flex flex-column flex-column-fluid">
        <div id="kt_app_toolbar" class="app-toolbar  d-flex flex-stack py-4 py-lg-8 ">
            <div class="d-flex flex-grow-1 flex-stack flex-wrap gap-2 mb-n10" id="kt_toolbar">
                ...
                @can('create workers')
                    <a href="{{ route('worker.add') }}" class="btn btn-sm primary-latest-container fw-bold">
...
@push('script')
    ...
    <script>
        var workerTable = $('#workerTable').DataTable({
            "pageLength": 25,
            "filter": true,
            "searching": true,
            "responsive": true,
        ...
        $('#search').keyup(function() {
            workerTable.search($(this).val()).draw();
        });
    </script>
    ...
    <script>
        $(document).on("click", ".delete-btn", function(event) {
            ...
            var id = $(this).data('id');
            var url = "{{ route('worker.delete', ':id') }}".replace(':id', id);
            Swal.fire({
                title: 'Are you sure?',
